...
const seedDatabase = async () => {
  try {
    ...
    seedUser.analysisCount = createdAnalyses.length;
    await seedUser.save();
    ...
    console.log(`   - Users: 1`);
    console.log(`   - Job Analyses: ${createdAnalyses.length}`);
    console.log('\n📈 Breakdown by Risk Level:');
    const riskCounts = createdAnalyses.reduce((acc, job) => {
      acc[job.riskLevel] = (acc[job.riskLevel] || 0) + 1;
      return acc;
    }, {});
    console.log(`   - 🔴 High Risk: ${riskCounts.High || 0} (Fake Jobs)`);
    console.log(`   - 🟢 Low Risk: ${riskCounts.Low || 0} (Legitimate Jobs)`);
